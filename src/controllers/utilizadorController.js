...
controller.listar = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Erro ao procurar o utilizador", error: error.message });
  }
};
...
controller.login = async (req, res) => {
  ...
  var utilizador = await Utilizador.findOne({ where: { email: email, ativo:true } })
    .then(function (data) {
      return data;
    })
    .catch((error) => {
    ...
    if (req.body.email && req.body.password && utilizador) {
      ...
      if (req.body.email === utilizador.email && isMatch) {
        ...
        res.json({
          ...
          token: token,
          id: utilizador.id,
          tipoutilizador: utilizador.tipoutilizadorid,
        });
      } else {
        res
          .status(403)
...
controller.listar_clientes = async (req, res) => {
  const data = await Utilizador.findAll({
    where: { tipoutilizadorid: { [Op.or]: [2, 3] } },
  })
    .then(function (data) {
      return data;
    })
...
controller.update = async (req, res) => {
  ...
  const id = req.params.id;
  const data = await Utilizador.update(req.body, { where: { id: id } });
  res.status(200).json({ success: true, message: "Utilizador atualizado" });
};
